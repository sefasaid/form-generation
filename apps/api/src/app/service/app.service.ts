...
import { PrismaService } from '@org/prisma-module';
import { EvaluationResult, Form, FormAnswer, FormSession, StepType } from '@org/prisma-module';
import { EvaluateBranchService } from './evaluate-branch.service';
import { FormStep, FormBranchRule } from '@org/prisma-module';
  ...
  async saveAnswer(sessionId: string, stepId: string, value: string | number | string[] | boolean, socketId?: string): Promise<{ success: boolean, nextStepId: string | null, end: EvaluationResult | null }> {
    ...
    if (!nextStepId) {
      if (!step.nextStepId) {
        ...
        if (optionalRuleResults.length > 0) {
