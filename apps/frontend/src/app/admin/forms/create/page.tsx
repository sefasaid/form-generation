...
import { StepType, Operator, EvaluationResult } from '../../../_types';
...
export default function CreateFormPage() {
  ...
  const updateOption = (stepIndex: number, optionIndex: number, updates: Partial<FormOption>) => {
    ...
    const step = newSteps[stepIndex];
    if (step.options) {
      step.options[optionIndex] = { ...step.options[optionIndex], ...updates };
    }
    setFormData({ ...formData, steps: newSteps });
  };
  ...
  const updateBranch = (stepIndex: number, branchIndex: number, updates: Partial<FormBranch>) => {
    ...
    const step = newSteps[stepIndex];
    if (step.branches) {
      step.branches[branchIndex] = { ...step.branches[branchIndex], ...updates };
    }
    setFormData({ ...formData, steps: newSteps });
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const response = await postData('/admin/forms/simple', formData);
